...
* When the stack depth reaches $16$, removing additional items from the stack does not change its depth. To keep the depth at $16$, $0$'s are inserted into the deep end of the stack for each removed item.

## Stack representation
The VM allocates $19$ trace columns for the stack. The layout of the columns is illustrated below.

![](https://hackmd.io/_uploads/B1ysymQkq.png)
...
Overall, the during a right shift we do the following:
...
We also need to make sure the the values in the columns representing the top $16$ items of the stack are updated correctly. In general, these constraints will be operation-specific (i.e., different operation would affect the top of the stack differently). But we can illustrate the general approach on the example of two operations: `DUP` and `DROP`.
